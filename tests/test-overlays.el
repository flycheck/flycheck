...
(ert-deftest flycheck-overlay-categories ()
  ...
  (should (eq (get 'flycheck-error-overlay 'face) 'flycheck-error-face))
  (should (eq (get 'flycheck-warning-overlay 'face) 'flycheck-warning-face))
  ...
  (with-temp-buffer
    ...
    (--each '(warning error)
      (pcase-let* ((overlay (flycheck-add-overlay (flycheck-error-new-at 1 1 it)))
        ...
        (should (eq face (get category 'face)))
