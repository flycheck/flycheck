...
;; Package-Requires: ((emacs "26") (dash "2.12.1") (pkg-info "0.4") (let-alist "1.0.4") (seq "1.11"))
...
Return the name of the temporary file."
  (let ((filename (funcall temp-file-fn (flycheck-buffer-file-local-name))))
...
_CHECKER is ignored."
  (-when-let* ((basefile (flycheck-buffer-file-local-name))
...
(defun flycheck--emacs-lisp-checkdoc-enabled-p ()
  ...
  (and (flycheck--emacs-lisp-enabled-p)
       ...
       (not (member (flycheck-buffer-file-local-name) '("Eldev" "Eldev-local")))))
