...
(defvar flycheck-buttercup-fixture-directory nil
  "Directory holding recorded checker output.

Defaults to a `fixtures' directory beside the specs.")

(defun flycheck-buttercup-fixture-file (checker resource)
...
flattened rather than read as directories."
  (expand-file-name
   ...
   (or flycheck-buttercup-fixture-directory
       ;; Beside the resources, not inside them: a fixture is recorded
       ;; output, not an input to a checker
       (expand-file-name
        "fixtures"
        (file-name-directory
         (directory-file-name flycheck-buttercup--resource-directory))))))

(defun flycheck-buttercup-fixture (checker resource)
  "Return CHECKER's recorded output over RESOURCE."
  (let ((file (flycheck-buttercup-fixture-file checker resource)))
